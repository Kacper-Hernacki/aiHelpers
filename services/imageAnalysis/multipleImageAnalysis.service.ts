  ...
  processMultipleImages: (
    ...
    try {
      ...
      const responseTimeout = setTimeout(() => {
        debug(
          `TIMEOUT: Processing taking too long, sending response after ${
            timeoutMs / 1000
          } seconds`
        );
        sendResponse();
        ...
        try {
          ...
          for (const imageData of imageBuffers) {
            try {
              ...
              if (
                sourcePlatform === "N/A" &&
                result.sourceIdentified &&
                result.sourceIdentified !== "N/A"
              ) {
                sourcePlatform = result.sourceIdentified;
              }
            } catch (analysisError: any) {
              debug(
                `Error analyzing image ${imageData.originalName}:`,
                analysisError
              );
            }
          }
          ...
          const textLines = consolidatedText
            .split("\n")
      ...
      bb.on("file", (field: string, file: any, info: any) => {
        ...
        file.on("close", async () => {
          try {
            ...
            // Upload to storage
            const result = (await digitalOceanService.uploadFile(
              buffer,
              filename,
              mimeType
            )) as any;
