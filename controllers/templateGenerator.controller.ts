  ...
  generateArticleThumbnail: async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        else if (Array.isArray(replicateResponse) && replicateResponse.length > 0) {
        ...
        else if (replicateResponse[0]?.status === 'processing' && replicateResponse[0]?.output) {
          imageUrl = replicateResponse[0].output;
        }
        // If it's the exact flux format from the example
        else {
          imageUrl = replicateResponse[0]?.output || replicateResponse[0]?.urls?.stream || '';
        }
      } catch (error) {
      ...
      const result = await socialCardTemplateService.createSocialCard(imageUrl, title);
